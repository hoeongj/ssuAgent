...
## 갱신 (2026-07-15) — 운영 키 누락 fail-closed

stable principal을 요청 바디로 수용한 뒤에는 공개 ingress에서 API 키가 빠지는 순간 비용 보호뿐 아니라
thread 소유권 신뢰 경계도 무너진다. 운영 Helm 값은 `AGENT_API_KEY_REQUIRED=true`와 non-optional Secret을
사용한다. required인데 키가 비어 있으면 lifespan 시작 전에 실패해 무인증 모드로 조용히 기동하지 않는다.
로컬 개발은 required flag 기본값 `false`를 유지해 명시적으로 게이트 없는 실행을 허용한다.
