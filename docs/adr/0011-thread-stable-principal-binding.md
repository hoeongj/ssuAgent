...
|---|---|
| 날짜 | 2026-07-09 |
| 상태 | Accepted — ssuAI server-verified principal 배선까지 활성화 |
...
`AgentRequest`/`ResumeRequest`에 `principal: str | None = None` 필드를 추가했다. 후속 ssuAI 배선은 browser access JWT를 same-origin agent proxy에만 전달하고, proxy가 ssuMCP `/api/auth/me`으로 검증한 `studentId`만 이 필드에 주입한다. browser request body의 `principal`은 계속 proxy에서 제거한다.
...
2. **ssuAgent는 신뢰된 프록시 요청만 수락해야 한다.** 현재 public ingress는 유지하지만 운영 `AGENT_API_KEY`와 `X-Agent-Key`를 강제해 ssuAI 서버 프록시만 `/agent/*`에 도달하게 한다. `AGENT_API_KEY_REQUIRED=true`인데 키가 없으면 startup을 거부한다. 무키 공개 POST가 가능하면 누구나 임의 principal을 주장할 수 있으므로 이 설계 전체가 무너진다.
3. **`sha256(principal)` 저장은 프라이버시 조치일 뿐 위조 방어가 아니다.** 학번은 열거 가능한(enumerable) 값이라 공격자가 임의 학번을 넣고 해시가 일치하도록 만드는 건 자명하다. 위조 방어는 오직 ①(프록시만 주입)과 ②(공개 ingress의 서비스 간 API 키 강제)에서 나온다 — 해시는 DB 유출 시 원문 노출을 막는 별개 목적이다.
...
동반 ssuAI 변경은 구현됐다. access JWT 검증이 성공한 호출은 principal 소유권으로 동작한다. Authorization이 없는 익명 호출만 기존 session/anonymous 경로를 사용하며, bearer 만료는 401, verifier 장애는 503으로 ssuAgent 호출 전에 종료한다. 운영 공개 경로는 무키 직접 호출 401과 Vercel 프록시 경유 payload 검증 422로 키 경계를 확인했다.

## 트레이드오프

- **얻는 것**: principal이 실제로 도착하므로 재로그인·멀티기기에서도 히스토리가 보존되고, ADR 0010의 세션-회전 결함이 사라진다. 원문 미저장으로 DB 유출 시 실제 신원 노출이 없다.
- **잃는 것 / 남는 리스크**:
  - **프록시·verifier 의존성**: 로그인 요청은 ssuAI 프록시와 ssuMCP `/api/auth/me`가 모두 가용해야 한다. 짧은 verifier timeout과 401/503 분리로 ownership 강등과 장기 대기를 막는다.
...
3. "공개 ingress를 유지하면서 principal을 어떻게 신뢰하나요? API 키 누락 시 왜 startup을 거부하나요?"
