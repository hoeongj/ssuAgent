...
> 갱신 (2026-07-12): 일반 clarification은 raw 메시지가 `[도서관 에이전트]`로
> 시작할 때만 도서관 연속 흐름으로 본다. 로그인 게이트 예외와 배경은 ADR 0019를 참조한다.
...
백엔드 예약 준비/대기 상태 문구는 도구 경로의 지시와 내부 식별자를 포함할 수 있으므로
사용자 표시용 문구로 그대로 쓰지 않는다. 예약 완료 문구는 agent 쪽에서 준비 응답의 좌석
설명과 대기 상태의 이용 시간처럼 구조적으로 파싱 가능한 조각을 조합해 만든다. 백엔드
원문은 ToolMessage와 모델 경로에 보존하고, 승인 카드 같은 사람에게 보이는 payload만
표시용으로 정리한다.
